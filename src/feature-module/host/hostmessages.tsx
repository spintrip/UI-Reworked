...
async function getHostProfileData() {
  const profile = await getHostProfile();
  hostID = profile.hostDetails.id;
}
const HostMessages: React.FC = () => {
  ...
  const chatData = useSelector((state: RootState) => state.HostChat);
  const HostData = useSelector((state: RootState) => state.hostData.profile);
  const { bookingId, id, messages } = chatData;
  const hostId = hostID;
  const profilePic = HostData.profilePic;
  ...
  const fetchMessages = async () => {
    try {
      ...
      dispatch({ type: "FETCH_HOST_MESSAGES_SUCCESS", payload: data });
    } catch (error) {
  ...
  useEffect(() => {
    const foregroundInterval = 3000; 
    const backgroundInterval = 180000; 
  ...
  const handleSendMessage = async () => {
    ...
    const messageData = {
      bookingId,
      senderId: hostId, // Host ID
      receiverId: id, // User ID
    ...
    try {
      ...
      dispatch({
        type: "FETCH_HOST_MESSAGES_SUCCESS",
  ...
  return (
    <div>
      <Helmet>
        <title>Host Messages | Spintrip Car Rentals</title>
        <meta
          name="description"
        ...
        <meta
          name="keywords"
          content="Spintrip Car Rentals host messages, communicate with renters, message support, car rental listings inquiries, car rental messages Bangalore"
        />
      </Helmet>
                        ...
                        <div className="chat-header">
                          <div className="notify-block d-flex">
                            <div className="media-img-wrap flex-shrink-0">
                              <div className="avatar">
                                <ImageWithBasePath
                                  src={profilePic ? profilePic : "/assets/img/profiles/avatar-02.jpg"}
                                  alt="User Profile"
                                />
                              </div>
                            </div>
                            <div className="media-body flex-grow-1">
                              <div className="user-name">Chat with User</div>
                            </div>
                          </div>
                        </div>
                              ...
                              {messages &&
                                messages.map(
                                  (msg: {
                                    ...
                                    <li
                                      key={msg.id}
                                      className={`notify-block ${msg.senderId === hostId ? "sent" : "received"} d-flex`}
                                    >
                                      <div className="avatar flex-shrink-0">
                                        <ImageWithBasePath
                                          src="/assets/img/profiles/avatar-03.png"
                                          alt="User Image"
                                          className={`avatar-img rounded-circle ${msg.senderId === hostId ? "d-none" : ""}`}
                                        />
                                      </div>
                                      <div className="media-body flex-grow-1">
                                        <div className="msg-box">
